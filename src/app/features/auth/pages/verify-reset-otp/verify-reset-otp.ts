  ...
  template: `
    <div class="auth-page">
      <div class="auth-card">
        ...
        <a routerLink="/auth/forgot-password" class="back-link"><fa-icon [icon]="['fas', 'arrow-left']" class="me-1"></fa-icon> {{ 'changeEmail' | translate }}</a>
      </div>
    </div>
  `,
  styles: [`
...
export default class VerifyResetOtpComponent implements OnDestroy, OnInit {
  ...
  onVerify(): void {
    ...
    this.authService.verifyPasswordResetOtp(this.email, this.otpCode)
  ...
  onResend(): void {
    ...
    this.authService.forgotPassword(this.email)
      ...
      .subscribe({
        ...
        error: (err: { message?: string }) => {
          this.resendLoading.set(false);
          this.error.set(err.message || 'Failed to resend confirmation.');
        },
      });
  }
}
